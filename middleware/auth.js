...
//func which hashes password and creates user object
const register = async(name, password, next) => {
    try {
        ...
        // password now hased - returned garbled string
        const passwordHash = await bcrypt.hash(password,salt);

        try {
            // create user
            const user = await User.create({name:name, passwordHash: passwordHash});
            // once creted, next() passes to next func in the chain -
...
const login = async(name, password, next) => {
    try {
        ...
        return match ? next(null, user) : next(null,null, {messgae:"Incorrect password"});
    } catch(error) {
        next (error);
    }
};
